...
{
    internal class TensorflowConcurrent
    {
        MainActivity activity = MainActivity.Context;
        public TensorflowConcurrent()
        {
        }
        ...
        public void ExecThreading(string fileFullPath, string fileName)
        {
            try
            {
                ...
                //SyncTasker(image.ImageInMatrixDictionaryConcurrent, output);
                //Forer(image.ImageInMatrixDictionaryConcurrent, output);
                Tasker(image.ImageInMatrixDictionaryConcurrent, output);
                //Parallelization(image.ImageInMatrixDictionaryConcurrent, output);


                ExportBitmapAsPNG(GetBitmap(Utils.Convert(output), 256), fileFullPath, fileName);
        ...
        private void Forer(ConcurrentDictionary<int, ConcurrentDictionary<int, ByteBuffer>> input, ConcurrentDictionary<int, ConcurrentDictionary<int, ByteBuffer>> output)
        {
        ...
        private void SyncTasker(ConcurrentDictionary<int, ConcurrentDictionary<int, ByteBuffer>> input, ConcurrentDictionary<int, ConcurrentDictionary<int, ByteBuffer>> output)
        {
            try
            {
                
                int tasks = 1;
                ConcurrentDictionary<int, ConcurrentDictionary<int, Map>> map = new ConcurrentDictionary<int, ConcurrentDictionary<int, Map>>();
                for (int i = 0; i < tasks; i++)
                {
                    map.TryAdd(i,new ConcurrentDictionary<int, Map>());
                }
                int curentTask = 0;
                for (int i = 0; i < input.Count; i++)
                {
                    for (int j = 0; j < input[0].Count; j++)
                    {
                        ...
                        map[curentTask].TryAdd(j,new Map() { X = i, Y = j });
        ...
        private void Tasker(ConcurrentDictionary<int, ConcurrentDictionary<int, ByteBuffer>> input, ConcurrentDictionary<int, ConcurrentDictionary<int, ByteBuffer>> output)
        {
        ...
        private void Parallelization(ConcurrentDictionary<int, ConcurrentDictionary<int, ByteBuffer>> input, ConcurrentDictionary<int, ConcurrentDictionary<int, ByteBuffer>> output)
        {
            int fullSize = input.Count * input[0].Count;
            int w = input.Count;
        ...
        private void ListingTensorflow(ConcurrentDictionary<int, ConcurrentDictionary<int, ByteBuffer>> input, ConcurrentDictionary<int, ConcurrentDictionary<int, ByteBuffer>> output,  ConcurrentDictionary<int, Map> map)
        {
            var model = new Xamarin.TensorFlow.Lite.Interpreter(GetModelAsMappedByteBuffer());
            model.AllocateTensors();
            var tensor = model.GetInputTensor(0);
            ...
            for (int i = 0; i < map.Count; i++)
            {
                int x = map[i].X;
                int y = map[i].Y;
                var buffer = Java.Nio.ByteBuffer.AllocateDirect(4 * 256 * 256 * 3);
                buffer.Order(ByteOrder.NativeOrder());
                model.Run(input[x][y], buffer);
                output[x][y]=   buffer;
            }
        ...
        {
            var model = new Xamarin.TensorFlow.Lite.Interpreter(GetModelAsMappedByteBuffer(),16);
            model.AllocateTensors();
            var tensor = model.GetInputTensor(0);
            var shape = tensor.Shape();
        ...
        //    return buffers;
        //}
        private ConcurrentDictionary<int, ConcurrentDictionary<int, ByteBuffer>> GetOutputByteBuffer(int width, int height)
        {
            ConcurrentDictionary<int, ConcurrentDictionary<int, ByteBuffer>> buffers = new ConcurrentDictionary<int, ConcurrentDictionary<int, ByteBuffer>>();
            for (int i = 0; i < width; i++)
            {
                buffers.TryAdd(i, new ConcurrentDictionary<int, ByteBuffer>());
                for (int j = 0; j < height; j++)
                {
                    buffers[i].TryAdd(j, Java.Nio.ByteBuffer.AllocateDirect(4 * 256 * 256 * 3)); 
                    buffers[i][j].Order(ByteOrder.NativeOrder());
                }
            }
            return buffers;
